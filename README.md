...
- **Search** one box across set names, artists, descriptions, big-moment notes,
  time of day and status. Multiple words all have to match.
- **Filter** by artist, energy level, time of day, status and minimum rating.
...
- **Go and listen.** Each set's main button names its platform — "Watch on
  ...
  Bandcamp and Vimeo are recognised; anything else says "Listen".
- **Share a link.** Filters live in the URL, so a filtered view can be sent or
  bookmarked. Each set also has a *Copy link to this set* button, which produces a
  link that opens straight into that set.

### Link previews

Pasting the site into Slack, iMessage, WhatsApp or Discord shows a title, blurb
and image rather than a bare URL. That image is `og.png`, generated by
`tools/make_og_image.py`, and `./refresh.sh` regenerates it so its "8 sets" line
stays accurate.

**One limitation worth knowing:** per-set links (`#set=...`) still preview as the
*site*, not as that individual set. Everything after the `#` never reaches the
server, so an unfurler can't see which set you meant. Giving each set its own
...
js/stats.js                   the summary arithmetic behind the stats panel
